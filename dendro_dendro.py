...
def get_GalaxyProps(galaxy, sampletable_file="./../../../data/sampletable/phangs_sample_table_v1p6.fits"):
...
def get_MuseProps(galaxy, muscat_file='/Users/abarnes/Dropbox/work/Projects/pressures/phangs/data/catalouge/v2/physprops/master_notmasked.fits'):
...
def get_maskedhdus(hdus, regions, hstha_hdu_name='hst07_hdus'):
...
def get_dedro(hdu, hdu_outmask, hdu_nomask, sampletable, min_npix=9, min_value_sig=3, min_delta_sig=3):
    ...
    pixsize = np.array([-1*hdu.header['CD1_1'], hdu.header['CD2_2']]).mean() * au.degree
    ...
    for i in range(min_value_sig_n):
        ...
        if props[i] is None: 
            ...
            if all_nan_check(hdus, regionID): 
                ...
                props[i] = get_dedro(hdus['hst07_hdus_smooth_masked'][0],
                                    hdus['hst07_hdus_smooth_maskedall'][0], 
                                    hdus['hst07_hdus_smooth'][0],
                                    sampletable=sampletable,
                                    min_npix=0, 
                                    min_value_sig=-100, 
                                    min_delta_sig=-100)[1]
...
def get_dedro_all_decorator(hdus, min_value_sig, sampletable, muscat_ids):
    ...
    for key in min_value_sig_str: 
        hdus['indexmap_trunk_hdus_%s' %key] = []
    ...
    for regionID in tqdm(range(n), desc='Dendrogram (with sigma ranges)', position=0):
        ...
        for i in range(min_value_sig_n): 
            try: 
                hdus['indexmap_trunk_hdus_%ssig' %min_value_sig[i]] += [indexmap_trunk_hdu[i].copy()]
            except: 
                ...
                hdus['indexmap_trunk_hdus_%ssig' %min_value_sig[i]] += [hdu_empty.copy()]
...
def save_fits(props_all):
    keys = list(props_all.keys())
    for key in keys: 
        filename = '../../analysis/dendro/props_all/01_props_all_%s.fits' %key
