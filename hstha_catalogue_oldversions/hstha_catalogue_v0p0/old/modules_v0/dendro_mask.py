...
def get_hdumask(hdus, hdu_ha, outputfile='tmp.fits', sig=3):
    """
    ...
    for hdu_id in tqdm(range(len(hdus['indexmap_trunk_hdus_%ssig' %sig])), desc='Masking regions'):

        # Extract the current HDU mask and its WCS
        hdu_mask = hdus['indexmap_trunk_hdus_3sig'][hdu_id]
        data_mask = (hdu_mask.data != -1)  # Convert to a Boolean mask
        shape_mask = data_mask.shape
        n_mask = np.sum(data_mask*1)  # Count of non-negative values in the mask
        wcs_mask = WCS(hdu_mask.header)

        # This file path seems unused in this context, but I'll leave it here as it was provided
        dir_hst = '../../../../ngc628c/data/hst_contsub/f658n_f555w_f814w/'
