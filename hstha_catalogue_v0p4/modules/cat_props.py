...
# Convert radius in arcsec to parsec
def calculate_radius(radius_arcsec, distance):
    pcperarcsec = distance.to('pc')/206265
    radius_pc = radius_arcsec.value * pcperarcsec
    return (radius_pc)


# Correct H-alpha flux for extinction
def correct_ha_flux(props_all, flux):
    correction_factor = (props_all['HA6562_FLUX_CORR_MUSE']/props_all['HA6562_FLUX_MUSE']).value
    corrected_flux_ha = flux * correction_factor
...
#         _, mask = misc.find_nearest(SB99models['time'].value, age[regionID].value)
